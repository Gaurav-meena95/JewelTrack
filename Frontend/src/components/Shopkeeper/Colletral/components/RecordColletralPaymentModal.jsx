...
const RecordColletralPaymentModal = ({
  show,
  onClose,
  item,
  editPaymentData,
    ...
    <div className='fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
      <div className='bg-card w-full max-w-md p-6 rounded-2xl border border-border/50 shadow-2xl'>
        <div className='flex justify-between items-center mb-5'>
          <h2 className='text-xl font-bold'>Record Loan Repayment</h2>
        ...
        <div className='bg-secondary/30 rounded-[8px] p-4 mb-5 space-y-1'>
          <p className='text-sm font-bold'>{item.customerId?.name}</p>
          <p className='text-xs text-muted-foreground'>
            Jewellery: {item.jewellery} ({item.weight}g)
          </p>
        ...
        <div className='space-y-4'>
          <div>
            <label className='text-sm font-medium text-muted-foreground mb-1 block'>
              Repayment Amount Now (₹)
            </label>
            <input
              type='number'
              placeholder='Enter amount being repaid...'
...
export default RecordColletralPaymentModal;
