...
const ImageUploadArea = ({ images, onImageUpload, onRemoveImage, label = "Reference Photos", showRequired = false }) => (
  <div className='bg-secondary/20 p-4 rounded-2xl border border-border/50'>
    <label className='text-sm font-bold flex items-center gap-2 mb-3'>
      <ImageIcon className='w-4 h-4 text-amber-500' /> {label} {showRequired && <span className='text-red-500 ml-1'>* Required</span>}
    </label>
    <div className='flex gap-4 overflow-x-auto pb-2'>
      ...
      <label className='shrink-0 w-20 h-20 rounded-[8px] border-2 border-dashed border-border/50 flex flex-col items-center justify-center text-muted-foreground hover:text-amber-500 hover:border-amber-400 cursor-pointer transition-colors'>
